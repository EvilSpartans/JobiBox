...
import { getJobiboxPortals, updateJobibox } from "../../services/jobibox.service";
...
export default function HomeScreen(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const fetchJobibox = async () => {
      try {
        ...
        if (!jobiboxId) {
          throw new Error("jobiboxId manquant dans le localStorage");
        }
        const response = await dispatch(getJobiboxPortals({ id: jobiboxId }));
        const portalsData = response.payload;
  
        if (portalsData && 'training' in portalsData && 'exam' in portalsData && 'version' in portalsData) {
          localStorage.setItem("trainingActivated", JSON.stringify(portalsData.training));
          localStorage.setItem("examActivated", JSON.stringify(portalsData.exam));
          ...
          console.error("Erreur API :", portalsData);
        }
  ...
  }, [navigate, dispatch, existingSelectedGreenFilter, expertInProgress, intermediateInProgress, beginnerInProgress]);
