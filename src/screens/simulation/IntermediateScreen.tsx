...
import { getQuestionVideos } from "../../services/questionVideo.service";
...
export default function IntermediateScreen(): React.JSX.Element {
  ...
  const questions = useSelector(
    (state: RootState) => state.questionVideo.questionVideos as QuestionVideo[],
  );
  const [categories, setCategories] = useState<QuestionList[]>([]);
  const [selectedCategory, setSelectedCategory] = useState("");
  const [loading, setLoading] = useState(true);
  const [mainQuestion, setMainQuestion] = useState<QuestionVideo | null>(null);
  ...
  const fetchQuestionVideos = async () => {
      const res = await dispatch(getQuestionVideos(token));
      return res.payload as QuestionVideo[];
  };
        ...
        <div className="min-h-screen w-full flex items-center justify-center overflow-hidden">
          <div className="flex flex-col justify-center min-h-[60%] h-fit tall:h-[90%] w-fit min-w-[60%] tall:w-[90%] space-y-8 tall:space-y-20 p-10 dark:bg-dark_bg_2 rounded-xl">
            ...
            <div className="text-center dark:text-dark_text_1">
              ...
              <img
                src="./public/images/intermediate.png"
                alt="Welcome"
