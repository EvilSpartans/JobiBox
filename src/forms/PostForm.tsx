...
import { sendConfirmNotification } from "../components/core/Notification";
...
import { ContractOption } from "../models/Contract";
import { EducationOption } from "../models/Education";
import { JobiBox } from "../models/JobiBox";
import { Portal } from "../models/Portal";
import { Post } from "../models/Post";

export default function PostForm(): React.JSX.Element {
  ...
  const { token } = user;
  const [videotheque, setVideotheque] = useState<boolean>(true);
  const [contracts, setContracts] = useState<any>([]);
  const [hidePortal, setHidePortal] = useState<boolean>(false);
  const [portals, setPortals] = useState<any>([]);
  const [portalsOptions, setPortalsOptions] = useState<{ value: string; label: string }[]>([]);
  const [categoryOptions, setCategoryOptions] = useState<{ value: string; label: string }[]>([]);
  const [startDate, setStartDate] = useState<Date | null>(null);
  const [photoFile, setPhotoFile] = useState(null);
  const [studiesOptions, setStudiesOptions] = useState<EducationOption[]>([]);
  ...
  const fetchPortals = async () => {
    try {
      ...
      const portalsData = response.payload as JobiBox;
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (res.payload && "title" in res.payload) {
        localStorage.setItem("urlQrcode", (res.payload as Post).video ?? "");
