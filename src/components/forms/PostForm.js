...
import { fetchCities, clearCities } from "../../store/slices/citySlice";
...
export default function PostForm() {
 ...
 const { cities, status: cityStatus } = useSelector((state) => state.city);
 ...
 useEffect(() => {
  const timeout = setTimeout(() => {
   if (cityQuery.trim().length > 0) {
    dispatch(fetchCities(cityQuery));
   } else {
   ...
   <div className="flex flex-col justify-center min-h-[60%] h-fit tall:h-[90%] w-fit min-w-[60%] tall:w-[90%] space-y-8 tall:space-y-2 p-10 dark:bg-dark_bg_2 rounded-xl">
    ...
    <div className="text-center dark:text-dark_text_1">
     ...
     <p className="mt-6 text-xl">
      <span className="text-blue-400">Complète les champs</span> suivants pour
      mettre en ligne ta vidéo.
     </p>
    </div>
