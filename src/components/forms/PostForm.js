...
export default function PostForm() {
    ...
    const fetchSubCategories = async () => {
        try {
            ...
            const updatedSubCategoryOptions = subCategoriesData.map((subCategory) => ({
                value: subCategory.name,
                label: subCategory.name,
            }));

            const filteredSubCategoryOptions = updatedSubCategoryOptions.filter(
                (subCategory) => subCategory.value !== "Actualité" && subCategory.value !== "Portail"
            );
   ...
   const [selectedVideo, setSelectedVideo] = useState(
    videoPath
        ? fetchVideoAndCreateFile(`https://test.jobissim.com/${videoPath}`, "video.mp4")
        : null
    );

    const fetchVideoAndSetSelected = async () => {
        const newSelectedVideo = videoPath
            ? await fetchVideoAndCreateFile(`https://test.jobissim.com/${videoPath}`, "video.mp4")
            : null;
