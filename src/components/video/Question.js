    ...
    const handleQuestionSelection = (question) => {
        if (currentTab === "Candidat") {
            if (selectedQuestionsCandidat.includes(question)) {
                setSelectedQuestionsCandidat(selectedQuestionsCandidat.filter((q) => q !== question));
            } else {
            ...
            if (selectedQuestionsRecruteur.includes(question)) {
                setSelectedQuestionsRecruteur(selectedQuestionsRecruteur.filter((q) => q !== question));
            } else {
                setSelectedQuestionsRecruteur([...selectedQuestionsRecruteur, question]);
            }
        }
    };

    // Determine the selected questions based on the current tab
    const selectedQuestions = currentTab === "Candidat" ? selectedQuestionsCandidat : selectedQuestionsRecruteur;
    ...
    return (
        <div className="w-full max-w-md space-y-8 p-10 dark:bg-dark_bg_2 rounded-xl">
            ...
            <div className="text-center dark:text-dark_text_1">
                ...
                <p className="mt-6 text-sm">Sélectionnez les questions auxquelles vous souhaitez répondre</p>
            </div>
            <div className="dark:text-dark_text_1">
                <div className="mb-6 flex justify-center">
                    <button
                        onClick={() => setCurrentTab("Candidat")}
                        className={`px-4 py-2 border-b-2 ${currentTab === "Candidat"
                    ...
                    <button
                        ...
                        className={`px-4 py-2 border-b-2 ${currentTab === "Recruteur"
                            ? "border-primary bg-primary text-white"
                            : "border-transparent bg-transparent text-gray-500"
                        }`}
                    >
            ...
            <button
                className={`w-full flex justify-center bg-blue_3 text-gray-100 p-4 rounded-full tracking-wide font-semibold focus:outline-none hover-bg-blue_4 shadow-lg cursor-pointer transition ease-in duration-300 ${selectedQuestions.length === 0 ? 'opacity-50 pointer-events-none' : ''}`}
                ...
                steps={[
                    {
                        ...
                        intro: 'Tu vas pouvoir sélectionner les questions auxquelles tu vas répondre dans un format de type question / réponse en vidéo.',
                    },
                    {
                        element: '.addButton',
                        intro: 'Si tu le souhaites, tu pourras également créer des questions afin de personnaliser ton CV vidéo du mieux possible.',
                    },
                    {
                        intro: "Information : Sélectionner les questions “peux-tu te présenter” et “quel poste cherches-tu et pourquoi” est fortement conseillé dans la création d’un CV vidéo."
                    }
                ]}
