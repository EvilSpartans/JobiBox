...
export default function Clip(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const removeCompiledVideo = async () => {
      try {
        await dispatch(deleteVideoProcess({ token, id: videoId! }));
      } catch (error) {
    ...
    if (videoId) {
      removeCompiledVideo().then(fetchQuestions);
    } else {
      fetchQuestions();
    }
  }, [dispatch, token]);

  const openShowModal = (index: any) => {
    const videoPath = `${BASE_URL}/${questions[index].video}`;
    console.log("🟢 Lecture de la vidéo :", videoPath);
  ...
  const openConfirmationModal = (index: any) => {
    setSelectedQuestionIndex(index);
  ...
  const handleTrim = async (index: number) => {
    closeTrimModal();
    let res: any;
    try {
      dispatch(changeStatus("loading"));
