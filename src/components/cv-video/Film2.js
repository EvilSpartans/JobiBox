...
export default function Film2({ onStartSequence }) {
  ...
  const initImageSegmenter = async () => {
  ...
  const handleApplyBackground = async (filter) => {
    if (!filter || !canvasRef.current || !videoCameraRef.current) return;

    // charger l’image de fond (comme avant)
    ...
    const checkVideoDimensions = async () => {
      ...
      if (video.videoWidth > 0 && video.videoHeight > 0) {
        ...
        // init du modèle récent si nécessaire
        if (!imageSegmenterRef.current) {
          const vision = await FilesetResolver.forVisionTasks(
            "https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@latest/wasm",
          );
        ...
        const render = () => {
          ...
          const result = imageSegmenterRef.current.segmentForVideo(
            video,
            performance.now(),
          );
          ...
          if (result?.categoryMask) {
            ...
            // =========================
            // 2️⃣ MASQUE (BORDS LISSES)
            // =========================
            ctx.globalCompositeOperation = "destination-in";
            ctx.filter = "blur(3px)"; // ← SEUL AJOUT IMPORTANT
            ctx.drawImage(maskCanvas, 0, 0, canvas.width, canvas.height);
            ctx.filter = "none";

            // =========================
            // 3️⃣ VIDÉO PAR-DESSUS (NORMALE)
            // =========================
